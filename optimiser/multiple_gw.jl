...
# ╔═╡ d8dd5852-2d87-499a-b1cc-a17a97f695ac
begin
...
function optimise_multiple_gameweek(df)
	...
	# Key diff: optimise for multiple gameweeks with declining weights
...
# ╠═╡ show_logs = false
team, captain_name, bench_names = optimise_multiple_gameweek(df); 
...
begin
	...
	for gw in START_GW:END_GW
        ...
        weighted = gw_points * GAMEWEEK_WEIGHTS[gw - START_GW + 1]
        total_weighted += weighted
        println("GW$gw: $(round(gw_points, digits=1)) pts × $(GAMEWEEK_WEIGHTS[gw - START_GW + 1]) = $(round(weighted, digits=1))")
    end
end
