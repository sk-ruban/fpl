...
md"# FPL Current Team with Transfers ⚽"
...
function optimise_from_current_team(df, current_team, initial_bank=0.0)
    ...
    current_team_buy_value = 0.0

	for item in current_team
	    if isa(item, Tuple)
	        name, pos = item
	        idx = findfirst(row -> row.Name == name && row.Pos == pos, eachrow(df))
	    else
	        name = item
	    ...
	    if idx !== nothing
	        ...
	        current_team_buy_value += df[idx, "BV"]
	    else
	        @warn "Player $name not found in data"
	    end
	end

	println("Current team value (selling): £$(round(current_team_sell_value, digits=1))m")
    println("Initial bank: £$(round(initial_bank, digits=1))m")
    println("Total available: £$(round(current_team_sell_value + initial_bank, digits=1))m")
	...
	# Squad init
	for i in 1:n
        @constraint(model, squad[i, START_GW] == 
            (i in current_team_indices ? 1 : 0) + transfer_in[i, START_GW] - transfer_out[i, START_GW])
    end
...
manifest_format = "2.0"
project_hash = "86dfff70b7ee4ccd92386a4b3f5ca6bee731ca6f"
